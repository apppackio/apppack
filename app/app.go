...
var ShellBackgroundCommand = []string{
	...
	strings.Join([]string{
		...
		"while true",
		"do EXECCMD=\"$(pgrep -fc ssm-session-worker\\ ecs-execute-command)\"",
...
func (a *App) CreateEcsSession(task ecs.Task) (*ecs.Session, error) {
	...
	execCmdInput := ecs.ExecuteCommandInput{
		Cluster:     task.ClusterArn,
		Command:     aws.String("su --preserve-environment --pty --command '/cnb/lifecycle/launcher bash -l' heroku"),
...
func (a *App) ConnectToEcsSession(ecsSession *ecs.Session, cmd *string) error {
