...
* Argument errors -- an unknown command, or a missing or malformed flag -- no longer print a second copy of the message to stdout. That copy landed in the JSON stream, so piping a mistyped command to `jq` could break on it.
